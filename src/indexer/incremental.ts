  ...
  try {
    ...
    for (const file of changedFiles) {
      try {
        ...
        const chunkDocs: Array<{ id: string; doc: CodeChunkDoc }> = chunks.map((chunk, index) => {
        ...
        if (file.status === 'added') result.filesAdded++;
        else result.filesModified++;
      } catch (err) {
