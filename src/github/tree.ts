...
export async function getDevHeadSha(owner: string, repo: string, targetBranch?: string): Promise<{ sha: string; branch: string }> {
...
export async function getTreeFiles(owner: string, repo: string, targetBranch?: string): Promise<TreeResult> {
  const octokit = await getOctokit();
